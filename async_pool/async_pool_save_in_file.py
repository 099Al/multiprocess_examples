...
import aiofiles
"""
Входные задачи делятся на порции, которые подаются 
на работу процедуре pool_task
Из расчета, что одну порцию деает один процесс

Далее эта порция в процессе раздается корутинам,
которые должны выполнять их асинхронно на одно процессоре
...
if __name__ == '__main__':
    ...
    with Pool(poolSize,initializer=init,initargs=(d,)) as pool:
