...
async def main():
    ...
    await asyncio.sleep(5)  #Чтобы к условию notify_all подошли, после, того как поставлен wait в функции task. Иначе некорректно отработает. Либо программа не завершрится.
    # acquire the condition
    async with condition:
